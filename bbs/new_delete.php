...
for($i=0;$i<$count_chk_bn_id;$i++)
{
    ...
    else // 코멘트 삭제
    {
        ...
        // 코멘트 삭제
        if (!delete_point($config['cn_id'], 'basic', $write['mb_id'], $bo_table, $comment_id, '댓글')) {
            insert_point($config['cn_id'], 'basic', $write['mb_id'], $board['bo_comment_point'] * (-1), "{$board['bo_subject']} {$write['wr_parent']}-{$comment_id} 댓글삭제");
        }

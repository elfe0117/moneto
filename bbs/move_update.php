...
// 원본 파일 디렉토리
$src_dir = G5_DATA_PATH.'/file/'.$config['cn_id'].'/'.$bo_table;
...
while ($row = sql_fetch_array($result))
{
    ...
    for ($i=0; $i<$count_chk_bo_table; $i++)
    {
        ...
        // 취약점 18-0075 참고
        $sql = "select * from {$g5['board_table']} where cn_id = '{$config['cn_id']}' AND bo_table = '".sql_real_escape_string($move_bo_table)."' ";
        $move_board = sql_fetch($sql);
        // 존재하지 않다면
        if( !$move_board['bo_table'] ) continue;

        $move_write_table = $g5['write_prefix'].$config['cn_id'].'_'.$move_bo_table;

        $src_dir = G5_DATA_PATH.'/file/'.$config['cn_id'].'/'.$bo_table; // 원본 디렉토리
        $dst_dir = G5_DATA_PATH.'/file/'.$config['cn_id'].'/'.$move_bo_table; // 복사본 디렉토리
        ...
        while ($row2 = sql_fetch_array($result2))
        {
            ...
            if (!$row2['wr_is_comment'])
            {
                ...
                $sql3 = " select * from {$g5['board_file_table']} where cn_id = '{$config['cn_id']}' AND bo_table = '$bo_table' and wr_id = '{$row2['wr_id']}' order by bf_no ";
                ...
                {
                    // 스크랩 이동
                    sql_query(" update {$g5['scrap_table']} set bo_table = '$move_bo_table', wr_id = '$save_parent' where cn_id = '{$config['cn_id']}' AND bo_table = '$bo_table' and wr_id = '{$row2['wr_id']}' ");

                    // 최신글 이동
                    sql_query(" update {$g5['board_new_table']} set bo_table = '$move_bo_table', wr_id = '$save_parent', wr_parent = '$save_parent' where cn_id = '{$config['cn_id']}' AND bo_table = '$bo_table' and wr_id = '{$row2['wr_id']}' ");

                    // 추천데이터 이동
                    sql_query(" update {$g5['board_good_table']} set bo_table = '$move_bo_table', wr_id = '$save_parent' where cn_id = '{$config['cn_id']}' AND bo_table = '$bo_table' and wr_id = '{$row2['wr_id']}' ");
                }
            }
            else
            {
                ...
                {
                    // 최신글 이동
                    sql_query(" update {$g5['board_new_table']} set bo_table = '$move_bo_table', wr_id = '$insert_id', wr_parent = '$save_parent' where cn_id = '{$config['cn_id']}' AND bo_table = '$bo_table' and wr_id = '{$row2['wr_id']}' ");
                }
            }
        ...
        sql_query(" update {$g5['board_table']} set bo_count_write = bo_count_write + '$count_write' where cn_id = '{$config['cn_id']}' AND bo_table = '$move_bo_table' ");
        sql_query(" update {$g5['board_table']} set bo_count_comment = bo_count_comment + '$count_comment' where cn_id = '{$config['cn_id']}' AND bo_table = '$move_bo_table' ");
...
if ($sw == 'move')
{
    for ($i=0; $i<count($save); $i++)
    {
        ...
        sql_query(" delete from {$g5['board_new_table']} where cn_id = '{$config['cn_id']}' AND bo_table = '$bo_table' and wr_id = '{$save[$i]['wr_id']}' ");
        sql_query(" delete from {$g5['board_file_table']} where cn_id = '{$config['cn_id']}' AND bo_table = '$bo_table' and wr_id = '{$save[$i]['wr_id']}' ");
    }

    // 공지사항이 이동되는 경우의 처리 begin
    $arr = array();
    $sql = " select bo_notice from {$g5['board_table']} where cn_id = '{$config['cn_id']}' AND bo_table = '{$bo_table}' ";
    $row = sql_fetch($sql);
    ...
    sql_query(" update {$g5['board_table']} set bo_notice = '{$bo_notice}', bo_count_write = bo_count_write - '$save_count_write', bo_count_comment = bo_count_comment - '$save_count_comment' where cn_id = '{$config['cn_id']}' AND bo_table = '$bo_table' ");
}

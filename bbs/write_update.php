...
if ($w == '' || $w == 'r') {
    ...
    // 새글 INSERT
    sql_query(" insert into {$g5['board_new_table']} ( cn_id, bo_table, wr_id, wr_parent, bn_datetime, mb_id ) values ( '{$config['cn_id']}', '{$bo_table}', '{$wr_id}', '{$wr_id}', '".G5_TIME_YMDHIS."', '{$member['mb_id']}' ) ");

    // 게시글 1 증가
    sql_query("update {$g5['board_table']} set bo_count_write = bo_count_write + 1 where cn_id = '{$config['cn_id']}' AND bo_table = '{$bo_table}'");
    ...
    if ($w == '') {
        ...
        insert_point($config['cn_id'], 'basic', $member['mb_id'], $board['bo_write_point'], "{$board['bo_subject']} {$wr_id} 글쓰기", $bo_table, $wr_id, '쓰기');
    } else {
        // 답변은 코멘트 포인트를 부여함
        // 답변 포인트가 많은 경우 코멘트 대신 답변을 하는 경우가 많음
        insert_point($config['cn_id'], 'basic', $member['mb_id'], $board['bo_comment_point'], "{$board['bo_subject']} {$wr_id} 글답변", $bo_table, $wr_id, '쓰기');
    }
}  else if ($w == 'u') {
...
@mkdir(G5_DATA_PATH.'/file/'.$config['cn_id'].'/'.$bo_table, G5_DIR_PERMISSION);
@chmod(G5_DATA_PATH.'/file/'.$config['cn_id'].'/'.$bo_table, G5_DIR_PERMISSION);
...
if(isset($_FILES['bf_file']['name']) && is_array($_FILES['bf_file']['name'])) {
    for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
        ...
        if (isset($_POST['bf_file_del'][$i]) && $_POST['bf_file_del'][$i]) {
            ...
            $row = sql_fetch(" select * from {$g5['board_file_table']} where cn_id = '{$config['cn_id']}' AND bo_table = '{$bo_table}' and wr_id = '{$wr_id}' and bf_no = '{$i}' ");

            $delete_file = run_replace('delete_file_path', G5_DATA_PATH.'/file/'.$config['cn_id'].'/'.$bo_table.'/'.str_replace('../', '', $row['bf_file']), $row);
        ...
        if (is_uploaded_file($tmp_file)) {
            ...
            if ($w == 'u') {
                ...
                if(isset($row['bf_file']) && $row['bf_file']){
                    $delete_file = run_replace('delete_file_path', G5_DATA_PATH.'/file/'.$config['cn_id'].'/'.$bo_table.'/'.str_replace('../', '', $row['bf_file']), $row);
                    if( file_exists($delete_file) ){
                        @unlink(G5_DATA_PATH.'/file/'.$config['cn_id'].'/'.$bo_table.'/'.$row['bf_file']);
                    }
                    // 이미지파일이면 썸네일삭제
                    if(preg_match("/\.({$config['cf_image_extension']})$/i", $row['bf_file'])) {
                        delete_board_thumbnail($bo_table, $row['bf_file']);
            ...
            $dest_file = G5_DATA_PATH.'/file/'.$config['cn_id'].'/'.$bo_table.'/'.$upload[$i]['file'];
...
for ($i=0; $i<count($upload); $i++)
{
    ...
    $row = sql_fetch(" select count(*) as cnt from {$g5['board_file_table']} where cn_id = '{$config['cn_id']}' AND bo_table = '{$bo_table}' and wr_id = '{$wr_id}' and bf_no = '{$i}' ");
    if ($row['cnt'])
    {
...
$row = sql_fetch(" select max(bf_no) as max_bf_no from {$g5['board_file_table']} where cn_id = '{$config['cn_id']}' AND bo_table = '{$bo_table}' and wr_id = '{$wr_id}' ");
for ($i=(int)$row['max_bf_no']; $i>=0; $i--)
{
    $row2 = sql_fetch(" select bf_file from {$g5['board_file_table']} where cn_id = '{$config['cn_id']}' AND bo_table = '{$bo_table}' and wr_id = '{$wr_id}' and bf_no = '{$i}' ");

    // 정보가 있다면 빠집니다.
    if (isset($row2['bf_file']) && $row2['bf_file']) break;

    // 그렇지 않다면 정보를 삭제합니다.
    sql_query(" delete from {$g5['board_file_table']} where cn_id = '{$config['cn_id']}' AND bo_table = '{$bo_table}' and wr_id = '{$wr_id}' and bf_no = '{$i}' ");
}

// 파일의 개수를 게시물에 업데이트 한다.
$row = sql_fetch(" select count(*) as cnt from {$g5['board_file_table']} where cn_id = '{$config['cn_id']}' AND bo_table = '{$bo_table}' and wr_id = '{$wr_id}' ");

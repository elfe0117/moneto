...
<nav id="gnb">
    ...
    <ul id="gnb_1dul">
        <?php
        // 1단계 분류 판매 가능한 것만
        $hsql = " select ca_id, ca_name from {$g5['g5_shop_category_table']} where cn_id = '{$channel['cn_id']}' AND length(ca_id) = '2' and ca_use = '1' order by ca_order, ca_id ";
        ...
        for ($i=0; $row=sql_fetch_array($hresult); $i++)
        {
            ...
            // 2단계 분류 판매 가능한 것만
            $sql2 = " select ca_id, ca_name from {$g5['g5_shop_category_table']} where cn_id = '{$channel['cn_id']}' AND LENGTH(ca_id) = '4' and SUBSTRING(ca_id,1,2) = '{$row['ca_id']}' and ca_use = '1' order by ca_order, ca_id ";

...
for($i=0; $i<$count; $i++) {
    ...
    if(!$row['qa_type'] && $row['qa_status']) {
        $row2 = sql_fetch(" select qa_content, qa_file1, qa_file2 from {$g5['qa_content_table']} where cn_id = '{$config['cn_id']}' AND qa_parent = '$qa_id' ");
        // 첨부파일 삭제
        for($k=1; $k<=2; $k++) {
        ...
        sql_query(" delete from {$g5['qa_content_table']} where cn_id = '{$config['cn_id']}' AND qa_type = '1' and qa_parent = '$qa_id' ");
    }

    // 답변글 삭제시 질문글의 상태변경
    if($row['qa_type']) {
        sql_query(" update {$g5['qa_content_table']} set qa_status = '0' where cn_id = '{$config['cn_id']}' AND qa_id = '{$row['qa_parent']}' ");
    }

    // 글삭제
    sql_query(" delete from {$g5['qa_content_table']} where cn_id = '{$config['cn_id']}' AND qa_id = '$qa_id' ");
}

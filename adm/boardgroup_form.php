...
if ($w == '') {
    ...
    $gr_id_attr = 'readonly';
    $gr = sql_fetch(" select * from {$g5['group_table']} where cn_id = '{$config['cn_id']}' AND gr_id = '$gr_id' ");
    $html_title .= ' 수정';
} else {
...
// 접근회원수
$sql1 = " select count(*) as cnt from {$g5['group_member_table']} where cn_id = '{$config['cn_id']}' AND gr_id = '{$gr_id}' ";
$row1 = sql_fetch($sql1);

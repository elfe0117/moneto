...
if ($w == '') {
    ...
    // 회원가입 포인트 부여
    insert_point($config['cn_id'], 'basic', $mb_id, $config['cf_register_point'], '회원가입 축하', '@member', $mb_id, '회원가입');

    // 추천인에게 포인트 부여
    if ($config['cf_use_recommend'] && $mb_recommend)
        insert_point($config['cn_id'], 'basic', $mb_recommend, $config['cf_recommend_point'], $mb_id.'의 추천인', '@member', $mb_recommend, $mb_id.' 추천');

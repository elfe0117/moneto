...
$po = sql_fetch(" select * from {$g5['poll_table']} where cn_id = '{$config['cn_id']}' AND po_id = '{$_POST['po_id']}' ");
...
if (!($search_ip || $search_mb_id)) {
    ...
    if ($is_member) { // 회원일 때는 id만 추가
        $mb_ids .= $member['mb_id'].',';
        $sql = " update {$g5['poll_table']} set po_cnt{$gb_poll} = po_cnt{$gb_poll} + 1, mb_ids = '$mb_ids' where cn_id = '{$config['cn_id']}' AND po_id = '$po_id' ";
    } else {
        $sql = " update {$g5['poll_table']} set po_cnt{$gb_poll} = po_cnt{$gb_poll} + 1, po_ips = '$po_ips' where cn_id = '{$config['cn_id']}' AND po_id = '$po_id' ";
    }

...
foreach($itemIds as $it_id) {
    ...
    if($it['ca_id']) {
        $cat = sql_fetch(" select ca_name from {$g5['g5_shop_category_table']} where cn_id = '{$channel['cn_id']}' AND ca_id = '{$it['ca_id']}' ");
        $ca_name = $cat['ca_name'];
    }
    if($it['ca_id2']) {
        $cat = sql_fetch(" select ca_name from {$g5['g5_shop_category_table']} where cn_id = '{$channel['cn_id']}' AND ca_id = '{$it['ca_id2']}' ");
        $ca_name2 = $cat['ca_name'];
    }
    if($it['ca_id3']) {
        $cat = sql_fetch(" select ca_name from {$g5['g5_shop_category_table']} where cn_id = '{$channel['cn_id']}' AND ca_id = '{$it['ca_id3']}' ");
